...
import com.gigaxysafe.remindmethere.databinding.ActivityMainBinding
...
class MainActivity : BaseActivity(), OnMapReadyCallback, GoogleMap.OnMarkerClickListener {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        if (requestCode == NEW_REMINDER_REQUEST_CODE && resultCode == Activity.RESULT_OK) {
            ...
            Snackbar.make(binding.MainContainer, R.string.reminder_added_success, Snackbar.LENGTH_LONG).show()
        }
    }
    ...
    private fun removeReminder(reminder: Reminder) {
        getRepository().remove(
            ...
            success = {
                showReminders()
                Snackbar.make(
                    binding.MainContainer, R.string.reminder_removed_success, Snackbar
                        .LENGTH_LONG
                ).show()
            },
            failure = {
                Snackbar.make(binding.MainContainer, it, Snackbar.LENGTH_LONG).show()
            })
    }
}